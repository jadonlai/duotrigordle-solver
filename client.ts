...
const PRACTICE_LINK_SELECTOR = 'div._tabWrapper_ag3fe_24 > div._tabContainer_ag3fe_30._shown_ag3fe_36 > div:nth-child(1) > button';
...
async function gather_results(page: Page): Promise<string[]> {
  ...
  const resultsPromises = boards.map(async board => {
    const cells = await board.$$eval(CELL_SELECTOR, els => els.map(el => el.className));
    const result = cells.slice(-10, -5);
    return result.map(cls => cls.includes("green") ? "f" : cls.includes("yellow") ? "p" : "i").join('');
  });
...
function solvedAmount(board: string[]): number {
  ...
  return (solve.match(/f/g) ?? []).length + 0.5 * (solve.match(/p/g) ?? []).length;
}
